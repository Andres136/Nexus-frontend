import useUserPermissions from "../../hooks/roles/useUserPermissions";
import { UserCheck, Users, Settings, Check, X, Save, Search, Filter } from "lucide-react";
import { useState } from "react";
import Select from "react-select";

export default function AsignarPermisosUsuario() {
  const {
    usuarios,
    permisos,
    selectedUser,
  ...
  const permisosAsignados = userPerms.length;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-xl sm:text-3xl font-bold text-gray-800">Asignar Permisos a Usuario</h1>
              <p className="text-sm sm:text-base text-gray-600">Gestiona los permisos individuales de cada usuario</p>
            </div>
          </div>

          {/* ✅ Selector de usuario mejorado */}
          <div className="bg-gradient-to-r from-gray-50 to-indigo-50 rounded-lg p-4 border border-indigo-200">
            ...
            <Select
              ...
              options={usuarios.map(u => ({ value: u.id, label: `${u.name} ${u.email ? `(${u.email})` : ''}` }))}
              value={usuarios.find(u => u.id == selectedUser)}
              onChange={(option) => {
                setSelectedUser(option.value);
                loadUserPerms(option.value);
              }}
            ...
            {usuarioSeleccionado && (
              <div className="mt-3 p-3 bg-white rounded-lg border border-indigo-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800">{usuarioSeleccionado.name}</p>
        ...
        {selectedUser && (
          <>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 text-sm w-full sm:w-64"
                    />
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 text-sm w-full sm:w-48"
                    >
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-6">
              <div className="bg-gradient-to-r from-gray-50 to-indigo-50 px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <h3 className="text-base sm:text-lg font-semibold text-gray-800">Permisos de Usuario</h3>
                <p className="text-xs sm:text-sm text-gray-600">
                  Mostrando {permisosFiltrados.length} de {totalPermisos} permisos para {usuarioSeleccionado?.name}
                </p>
              </div>
                  ...
                  <div key={p.id} className="p-4 hover:bg-gray-50 transition-colors duration-200">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 truncate font-mono bg-gray-100 px-2 py-1 rounded mt-1">
                          {p.path}
                        </p>
                      </div>
                      <div className="flex items-center gap-3 ml-3">
                        ...
                        <label className="flex items-center cursor-pointer">
                          <input
                            ...
                            onChange={() => togglePermiso(p.path)}
                            className="sr-only"
                          />
                          <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                            userPerms.includes(p.path) ? 'bg-gradient-to-r from-indigo-600 to-purple-600' : 'bg-gray-300'
                          }`}>
                            <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-lg transition-transform ${
                              userPerms.includes(p.path) ? 'translate-x-6' : 'translate-x-1'
                            }`} />
                          </div>
                        </label>
                      </div>
              ...
              <div className="hidden lg:block overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gradient-to-r from-gray-800 to-indigo-800 text-white">
                    <tr>
            ...
            <div className="fixed bottom-6 right-6 lg:relative lg:bottom-auto lg:right-auto">
              <button
                onClick={guardar}
                className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white px-6 py-3 rounded-full lg:rounded-lg font-semibold transition-all duration-200 transform hover:scale-105 shadow-2xl lg:shadow-lg text-sm lg:text-base"
              >
        ...
        {!selectedUser && (
          <div className="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">Selecciona un usuario</h3>
            <p className="text-gray-500">
              Elige un usuario de la lista superior para gestionar sus permisos individuales
            </p>
          </div>
        )}
      </div>
