...
import RegisterTipoContrato from "../../components/nomina/RegisterTipoContrato";
import { useGetTipoContrato } from "../../hooks/nomina/useGetTipoContrato";

export default function PageTipoContrato() {
  const { tipoContratos, isLoading } = useGetTipoContrato();
  const [modalOpen, setModalOpen] = useState(false);
  const [selectedId, setSelectedId] = useState(null);

  const lista = tipoContratos?.data ?? [];
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Gestiona los tipos de contrato disponibles.</p>
        </div>
        <button
          onClick={openCreate}
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {lista.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 font-mono text-xs text-gray-500">{item.codigo}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 p-8 animate-slide-in">
            ...
            <RegisterTipoContrato id={selectedId} onClose={closeModal} />
          </div>
        </div>
      )}
    </div>
