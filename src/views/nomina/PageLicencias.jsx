    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
        <h3 className="text-base font-semibold text-gray-800 mb-1">
          {accion === "aprobar" ? "Aprobar licencia" : "Rechazar licencia"}
        </h3>
...
export default function PageLicencias() {
  ...
  const handleGestion = async (observacion) => {
    ...
    try {
      const fn = accion === "aprobar" ? licenciaService.aprobar : licenciaService.rechazar;
    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-sm text-gray-400">No hay licencias registradas.</div>
        ) : (
            ...
            <tbody className="bg-white divide-y divide-gray-50">
              {lista.map((item) => (
                <tr key={item.uuid} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-gray-600 whitespace-nowrap">{item.inicio?.slice(0, 10) ?? "-"}</td>
                  <td className="px-4 py-3.5 text-gray-600 whitespace-nowrap">{item.fin?.slice(0, 10) ?? "-"}</td>
                  <td className="px-4 py-3.5 text-gray-700 font-medium">{item.dias_calendario ?? "-"}</td>
