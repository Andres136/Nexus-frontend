...
import { useGetHorasExtras } from "../../hooks/nomina/useGetHorasExtras";
...
export default function PageHorasExtras() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [gestion, setGestion] = useState(null);
  const [loadingUuid, setLoadingUuid] = useState(null);

  const { horasExtras, isLoading } = useGetHorasExtras({ search: search || undefined });
  const lista = horasExtras?.data?.data ?? horasExtras?.data ?? [];
  ...
  const handleGestion = async (observacion) => {
    ...
    try {
      ...
      showToast("error", "Error al procesar la hora extra");
    } finally {
      setLoadingUuid(null);
      setGestion(null);
    }
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-xl font-semibold text-gray-800">Horas Extras</h1>
          <p className="text-sm text-gray-500 mt-0.5">Gestiona las solicitudes de horas extras del personal.</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-sm text-gray-400">No hay horas extras registradas.</div>
        ) : (
          <table className="min-w-full divide-y divide-gray-100 text-sm">
            <thead className="bg-gray-50">
              <tr>
                {["Empleado","Fecha","Tipo","Horas","Motivo","Estado","Gestionado por","Acciones"].map((h) => (
