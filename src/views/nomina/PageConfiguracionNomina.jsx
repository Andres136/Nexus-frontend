...
export default function PageConfiguracionNomina() {
  ...
  useEffect(() => {
    ...
    setAjusteForm((prev) => ({
      ...prev,
      salario_nuevo: prev.salario_nuevo || contratoSeleccionado.base_salario || "",
      auxilio_nuevo: prev.auxilio_nuevo || contratoSeleccionado.auxilio_transporte || "",
      no_salarial_nuevo: prev.no_salarial_nuevo || contratoSeleccionado.no_salarial || 0,
    }));
  ...
  const handleAjuste = (event) => {
    ...
    setAjusteForm((prev) => {
      ...
      if (name === "contratacion_id") {
        const contrato = contratosLista.find((item) => String(item.id) === String(value));
      ...
      if (name === "porcentaje_aumento" && base > 0) {
        ...
        next.salario_nuevo = Math.round(base * (1 + Number(value || 0) / 100));
      }
  ...
  const guardarAjusteSalarial = (event) => {
    ...
    ajusteMutation.mutate({
      ...
      porcentaje_aumento: ajusteForm.porcentaje_aumento === "" ? null : Number(ajusteForm.porcentaje_aumento),
      status: true,
    });
  };
  ...
  return (
    <div className="p-6 max-w-screen-xl mx-auto">
      ...
      <form onSubmit={guardarAjusteSalarial} className="mb-5 rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Ajustes salariales por vigencia</h2>
            <p className="text-sm text-gray-500">Configura salario mínimo, auxilio de transporte y aumentos especiales por empleado.</p>
          </div>
          <button
            type="submit"
        ...
        <div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1fr)_420px]">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <label className="block md:col-span-2 xl:col-span-1">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Empleado / contrato</span>
              <select
            ...
            <label className="block">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Tipo de ajuste</span>
              <select
                name="tipo_ajuste"
            ...
            <label className="block">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Aplica desde</span>
              <input
                type="date"
                name="fecha_vigencia"
            ...
            <label className="block">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Nuevo salario</span>
              <input
                type="number"
                min="0"
                step="1"
            ...
            <label className="block">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">No salarial</span>
              <input
                type="number"
                min="0"
                step="1"
            ...
            <label className="block">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Aumento %</span>
              <input
                type="number"
                min="0"
                step="0.01"
            ...
            <label className="block md:col-span-2 xl:col-span-3">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Observación</span>
              <textarea
                name="observacion"
          ...
          <div className="rounded-lg border border-gray-100 bg-gray-50">
            <div className="border-b border-gray-100 px-4 py-3">
              ...
              <p className="text-xs text-gray-500">Estos valores ya alimentan nómina, prestaciones y retiro.</p>
            </div>
            <div className="max-h-[360px] overflow-auto">
              {loadingAjustes ? (
                <div className="p-4 text-sm text-gray-500">Cargando ajustes...</div>
              ) : ajustesLista.length ? (
                ajustesLista.map((ajuste) => (
                  <div key={ajuste.uuid} className="border-b border-gray-100 bg-white px-4 py-3 last:border-b-0">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-gray-900">
                          {ajuste.empleado?.name || ajuste.contratacion?.usuario?.name || "Empleado"}
                        </p>
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded-md bg-gray-50 px-2 py-1">
                        ...
                        <p className="font-semibold text-gray-700">{formatCOP(ajuste.salario_anterior)}</p>
                      </div>
                      <div className="rounded-md bg-emerald-50 px-2 py-1">
                        <span className="text-emerald-600">Nuevo</span>
                        <p className="font-semibold text-emerald-700">{formatCOP(ajuste.salario_nuevo)}</p>
                      </div>
                    </div>
                  </div>
                ))
              ) : (
                <div className="p-4 text-sm text-gray-500">Aún no hay ajustes salariales registrados.</div>
              )}
            </div>
          </div>
        </div>
