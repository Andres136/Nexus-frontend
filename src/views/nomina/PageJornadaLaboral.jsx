...
export default function PageJornadaLaboral() {
  ...
  const lista = jornadas?.data?.data ?? [];
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-sm text-gray-400">
            No hay jornadas laborales registradas.
          </div>
        ) : (
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {lista.map((item) => (
                <tr key={item.uuid} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-600">{item.horas_semanales} hrs</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-8 animate-slide-in">
            ...
            <RegisterJornadaLaboral uuid={selectedUuid} onClose={closeModal} />
          </div>
        </div>
      )}
    </div>
