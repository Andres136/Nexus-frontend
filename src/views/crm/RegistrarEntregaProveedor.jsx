...
function toDatetimeLocal(dateString) {
  ...
  return d.toISOString().slice(0, 10); // Solo YYYY-MM-DD
}
...
export default function RegistrarEntregaProveedor({ modo = "crear" }) {
  ...
  const {bodegas}=useSedes();//User trae id de sede
...
const abrirHistorial = (entregas, detalleId) => {
  const entregasConDetalle = entregas.map(e => ({
    ...
    producto_id: e.producto_id, // Asegura que producto_id esté incluido
  }));
  console.log("Entregas con detalle_id:", detalleId);
...
const cerrarModal = () => {
    ...
    setHistorialEntregas([]); // Limpiar el historial al cerrar
  };
      ...
      const entregas = detalles
        ...
        .map((d, i) => {
          ...
          return {
            ...
            empresa_id: orden.empresa?.id || null, // asegúrate que orden trae empresa
            sede_id: user.sede_id, // desde usuario autenticado
            stock: cant // cantidad a agregar al inventario
...
// Convertir a números o null antes de comparar
      ...
      inventario: {
        ...
        bodega_id: bodegaSeleccionada?.value,
        empresa_id: orden.empresa?.id, // asegúrate que orden trae empresa
        producto_id: entrega.producto_id, // relaciona el producto al item
  ...
  inventario: {
    ...
    console.log(error.response.data.errors); // ← aquí llegan todos los errores
  ...
  //REcargar detalles después  de editar actualizacion de entrega
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
           ...
           <p className="text-gray-900 text-sm font-extrabold">{orden.proveedor_nombre}</p>
          </div>
...
/>
            ...
            {errorBodega && <p className="text-red-500 text-xs mt-1">{errorBodega}</p>}
  ...
  {orden.observaciones && (
    <div className="mt-6 bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-md shadow-sm">
      <p className="text-yellow-800 text-sm leading-relaxed whitespace-pre-line">
        <strong className="font-semibold text-yellow-900">Observaciones:</strong>{" "}
        {orden.observaciones}
      </p>
    </div>
  )}
...
<div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
  <div className="overflow-x-auto">
    <table className="w-full divide-y divide-gray-200" style={{ minWidth: '1400px' }}> {/* ✅ REDUCIDO: de 1800px a 1400px */}
      <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
        <tr>
          ...
          <th className="px-2 py-2 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-48"> {/* ✅ REDUCIDO: de 64 a 48 */}
            Descripción
          </th>
          ...
          <th className="px-2 py-2 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-36"> {/* ✅ REDUCIDO: de 44 a 36 */}
            Proveedor
          </th>
          <th className="px-2 py-2 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-32"> {/* ✅ REDUCIDO: de 40 a 32 */}
            Proceso
          </th>
          <th className="px-2 py-2 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-24"> {/* ✅ REDUCIDO: de 80 a 24 */}
            Historial
          </th>
          <th className="px-2 py-2 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-36"> {/* ✅ REDUCIDO: de 48 a 36 */}
            Nueva Entrega
          </th>
          <th className="px-2 py-2 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-32"> {/* ✅ REDUCIDO: de 44 a 32 */}
            Observaciones
          </th>
        </tr>
      </thead>
          ...
          return (
            <tr key={key} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-2 py-2 whitespace-nowrap text-center w-12">
                <div className="flex items-center justify-center w-6 h-6 bg-blue-100 rounded-full"> {/* ✅ REDUCIDO: de 8x8 a 6x6 */}
                  <span className="text-xs font-bold text-blue-600">{detalle.item}</span> {/* ✅ REDUCIDO: de text-sm a text-xs */}
                </div>
              </td>

              {/* Código - MÁS COMPACTO */}
              <td className="px-2 py-2 whitespace-nowrap text-center w-16">
                <span className="text-xs text-gray-700 font-mono">{detalle.code || '—'}</span>
              </td>
              ...
              <td className="px-2 py-2 w-20">
                <div className="relative">
                  ...
                  />
                  <span className="absolute -bottom-0.5 right-0.5 text-xs text-gray-400">kg</span>
                </div>
              </td>
              ...
              <td className="px-2 py-2 whitespace-nowrap w-20">
                <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${estado.color} text-white shadow-sm`}> {/* ✅ COMPACTO: px-2 py-1→px-1.5 py-0.5 */}
                  <div className="w-1 h-1 bg-white rounded-full mr-1"></div> {/* ✅ COMPACTO: w-1.5 h-1.5→w-1 h-1 */}
                  {estado.texto}
                </span>
              </td>
              ...
              <td className="px-2 py-2 w-36">
                {detalle.cantidad_entregada >= detalle.cantidad_solicitada ? (
                  <div className="relative">
                    <div className="bg-gray-100 border border-gray-300 rounded-md px-2 py-1 text-xs text-gray-600 flex items-center"> {/* ✅ COMPACTO */}
                    ...
                    <Select
                      ...
                      styles={{
                        control: (provided) => ({
                          ...provided,
                          borderColor: '#d1d5db',
                          borderRadius: '0.375rem', /* ✅ COMPACTO: 0.5rem→0.375rem */
                          fontSize: '0.75rem',
                          minHeight: '28px' /* ✅ COMPACTO: 32px→28px */
                        }),
                        option: (provided) => ({
                          ...provided,
                          fontSize: '0.75rem'
              ...
              <td className="px-2 py-2 w-24">
                {detalle.entregas && detalle.entregas.length > 0 ? (
                  <button
                    onClick={() => abrirHistorial(detalle.entregas, detalle.id,)}
                  ...
                  >
                    <svg className="w-3 h-3 mr-1 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 10h11M9 21V3m0 0L5 7m4-4l4 4m6 6h2a2 2 0 012 2v6a2 2 0 01-2 2h-2m-2 4l-4-4m4 4l4-4" />
                    </svg>
                    Ver ({detalle.entregas.length}) {/* ✅ COMPACTO: texto reducido */}
                  </button>
                ) : (
              ...
              <td className="px-2 py-2 w-36">
                <div className="bg-green-50 rounded-md p-2"> {/* ✅ COMPACTO: rounded-lg→md, p-3→p-2 */}
                  <div className="space-y-1.5"> {/* ✅ COMPACTO: space-y-2→space-y-1.5 */}
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-0.5">Cantidad (kg)</label> {/* ✅ COMPACTO: mb-1→mb-0.5 */}
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                    ...
                    <div>
                      ...
                      />
                      {erroresFecha[index] && (
                        <p className="text-red-500 text-xs mt-0.5">⚠️ {erroresFecha[index]}</p>
                      )}
                    </div>
                  </div>
                </div>
...
>
  <h2 className="text-2xl font-bold mb-4 text-gray-900">Historial de Entregas</h2>

  {historialEntregas.length === 0 ? (
    <p className="text-gray-600">No hay entregas registradas para este ítem.</p>
  ) : (
            ...
            <tr key={entrega.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    nuevas[idx] = { ...nuevas[idx], cantidad_entregada: e.target.value };
              ...
              <td className="px-4 py-2">
                <input
                  ...
                  onChange={(e) => {
                    const nuevas = [...historialEntregas];
                    nuevas[idx] = { ...nuevas[idx], fecha_entrega: e.target.value };
                    setHistorialEntregas(nuevas);
                  }}
  ...
  {entrega.bodega_id ? (
    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700 mr-2">
      {bodegas.find((b) => b.id === entrega.bodega_id)?.nombre || "Desconocida"}
    </span>
  ) : (
    <span className="text-xs text-gray-400 italic">Sin bodega</span>
  )}
  ...
  <Select
    ...
    value={
      entrega.bodega_id
        ? { value: entrega.bodega_id, label: bodegas.find(b => b.id === entrega.bodega_id)?.nombre || "Desconocida" }
        : null
    }
    onChange={(selected) => {
      const nuevas = [...historialEntregas];
  ...
  />
              ...
              <td className="px-4 py-2">
                <input
                  ...
                  onChange={(e) => {
                    const nuevas = [...historialEntregas];
                    nuevas[idx] = { ...nuevas[idx], observaciones: e.target.value };
                    setHistorialEntregas(nuevas);
                  }}
      ...
      onClick={async () => {
        try {
          ...
          for (const entrega of historialEntregas) {
            ...
            const detalleIndex = nuevosDetalles.findIndex((d) => d.id === entrega.detalle_id);
            if (detalleIndex !== -1) {
              nuevosDetalles[detalleIndex].entregas = nuevosDetalles[detalleIndex].entregas.map(
                (e) => (e.id === entrega.id ? entrega : e)
              );
