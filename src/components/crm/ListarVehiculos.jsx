...
export default function ListarVehiculos() {
  ...
  const {
    ...
    fotosPorVehiculo, abrirLightbox,
    openLightbox, galeria, cerrarLightbox,
    ...
    <div className="p-4 grid grid-cols-1 gap-4">
      ...
      {loading ? (
        ...
        <>
            ...
            return (
              <div key={vehiculo.id} className="border rounded mb-4 shadow">
                ...
                >
                  {/* Foto */}
                  <img
                    src={encodeURI(`${import.meta.env.VITE_API_URL}/storage/${vehiculo.foto}`)}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs mt-1">
                      <span className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Tipo: {vehiculo.tipo}</span>
                      <span className="bg-red-100 text-red-800 px-2 py-0.5 rounded">Modelo: {vehiculo.modelo}</span>
                      <span className="bg-orange-100 text-orange-800 px-2 py-0.5 rounded">Marca: {vehiculo.marca}</span>
                      <span className="bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">Licencia: {vehiculo.licencia_transito}</span>
                      <span className="bg-purple-100 text-purple-800 px-2 py-0.5 rounded">Conductor: {vehiculo.conductor}</span>
                      <span className="bg-green-100 text-green-800 px-2 py-0.5 rounded">Año: {vehiculo.anio}</span>
                      <span className="bg-gray-100 text-gray-800 px-2 py-0.5 rounded">Estado: {vehiculo.estado}</span>
                      <span className="bg-purple-100 text-purple-800 px-2 py-0.5 rounded">
                        Km actual: {vehiculo.kilometraje_actual?.toLocaleString("es-CO")}
                      </span>
                      <span className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Nombre: {vehiculo.nombre}</span>
                      <span className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Servicio: {vehiculo.tipo_servicio}</span>
                      <span className="bg-red-100 text-red-800 px-2 py-0.5 rounded">Color: {vehiculo.color}</span>
                      <span className="bg-orange-100 text-orange-800 px-2 py-0.5 rounded">Carrocería: {vehiculo.tipo_carroceria}</span>
                      <span className="bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">Combustible: {vehiculo.tipo_combustible}</span>
                      <span className="bg-purple-100 text-purple-800 px-2 py-0.5 rounded">Motor: {vehiculo.numero_motor}</span>
                      <span className="bg-green-100 text-green-800 px-2 py-0.5 rounded">Chasis: {vehiculo.numero_chasis}</span>
                      <span className="bg-gray-100 text-gray-800 px-2 py-0.5 rounded">Propietario: {vehiculo.propietario}</span>
                      <span className="bg-purple-100 text-purple-800 px-2 py-0.5 rounded">Identificación: {vehiculo.identificacion}</span>
                      <span className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Organismo: {vehiculo.organismo_transito}</span>
                      <span className="bg-red-100 text-red-800 px-2 py-0.5 rounded">F. matrícula: {vehiculo.fecha_matricula}</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 text-xs mt-2">
                      ...
                      {resumen.masUrgente && (
                        ...
                        >
                          🔔 Más urgente: {resumen.masUrgente.tipo} · {new Date(resumen.masUrgente.fecha).toLocaleDateString('es-CO')}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap justify-end gap-3 mt-4">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); navigate(`/auth/crm/vehiculos/${vehiculo.id}/editar`); }}
                    ...
                    {user?.role_id === 1 && (
                      <button
                        onClick={(e) => { e.stopPropagation(); eliminarVehiculo(vehiculo.id); }}
                ...
                {expanded === vehiculo.id && (
                  <div className="p-4 bg-white text-sm space-y-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                          {fotosPorVehiculo[vehiculo.id].map((foto) => (
                            <img
                              ...
                              alt={`Foto ${foto.id}`}
                              onClick={() => {
                                const slides = (fotosPorVehiculo[vehiculo.id] || []).map((f) => ({
                    ...
                    {["mantenimientos", "documentos", "inspecciones"].map((seccion) => (
                      <div key={seccion} className="mt-4">
                        <h4 className="font-semibold mb-2">
                          {seccion === "mantenimientos" && "🛠️ Mantenimientos"}
                        ...
                        <input
                          ...
                          onChange={(e) => actualizarFecha(vehiculo.id, seccion, e.target.value)}
                        ...
                        {seccion === "documentos" && (() => {
                          ...
                          const urgentes = docs
                            .filter(d => ["VENCIDO","POR_VENCER"].includes(estadoDoc(d.fecha_vencimiento)))
                          ...
                          return urgentes.length > 0 ? (
                            <div className="mb-2 p-2 rounded bg-amber-50 border border-amber-200">
                              <div className="flex items-center justify-between">
                                <strong className="text-amber-800">⚠️ Documentos urgentes</strong>
                              ...
                              <ul className="mt-1 text-xs text-amber-900 space-y-1">
                                {urgentes.map((d) => (
                                  <li key={d.id} className="flex items-center justify-between">
                                    <span>
                                      {d.tipo_documento} · vence {new Date(d.fecha_vencimiento).toLocaleDateString("es-CO")}
                                    </span>
                                    <span
                                    ...
                                    >
                                      {diasHasta(d.fecha_vencimiento) < 0
                                        ? "VENCIDO"
                                        : `${diasHasta(d.fecha_vencimiento)} días`}
                                    </span>
                                  </li>
                                ))}
                              </ul>
                                    ...
                                    <>
                                      ...
                                      <td>
                                        {item.fecha_realizado ?? (
                                          <i className="text-gray-400">Pendiente</i>
                                        )}
                                      </td>
                                        ...
                                        {item.archivo ? (
                                          item.archivo.endsWith(".pdf") ? (
                                            ...
                                            <img
                                              ...
                                              alt="Soporte"
                                              onClick={() => {
                                                abrirLightbox([{ src: `${import.meta.env.VITE_API_URL}/storage/${item.archivo}` }]);
                                              }}
                                    ...
                                    <>
                                      ...
                                      <td>
                                        <input
                                          ...
                                          value={
                                            (item.id && item.id in ({})) // nada, evita warning por precedence
                                          ...
                                          defaultValue={item.fecha_vencimiento}
                                        />
                                      </td>
                                      <td>
                                        <input
                                          type="date"
                                          className="border rounded p-1 text-sm"
                                          value={item.fecha_renovacion || ""}
                                          onChange={(e) =>
                                            manejarCambioFecha(item.id, "fecha_renovacion", e.target.value)
                                          }
                                          defaultValue={item.fecha_renovacion}
                                        />
                                      </td>
                                      <td>
                                        {(() => {
                                          const est = estadoDoc(item.fecha_vencimiento);
                                          if (est === "VENCIDO") {
                                      ...
                                      <td>
                                        {item.documento_pdf ? (
                                          <a
                                            href={encodeURI(`${import.meta.env.VITE_API_URL}/storage/${item.documento_pdf}`)}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="underline"
                                          >
          ...
          {/* Lightbox global */}
          <Lightbox open={openLightbox} close={cerrarLightbox} slides={galeria} />
          ...
          <div className="flex justify-center gap-4 mt-4">
            ...
            <button
              onClick={() => setPage((prev) => (prev < vehiculos.last_page ? prev + 1 : prev))}
